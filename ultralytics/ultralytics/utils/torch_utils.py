...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # Create EMA model using state_dict to avoid deepcopy issues
        model = unwrap_model(model)
        ch = model.yaml.get('channels', 3)
        nc = model.yaml.get('nc', None)
        if hasattr(model, 'ii_target'):
            # For YOLADetectionModel
            self.ema = model.__class__(cfg=model.yaml, ch=ch, nc=nc, verbose=False, ii=model.ii_target, ii_warmup=model.ii_warmup)
        else:
            self.ema = model.__class__(cfg=model.yaml, ch=ch, nc=nc, verbose=False)
        self.ema.load_state_dict(model.state_dict())
        self.ema.eval()
        device = next(model.parameters()).device
        self.ema.to(device)  # Move to same device as model
